...
const SoupStation = (props) => {
    const history = useHistory();
    const pathname = sessionStorage.getItem('pathname')
    const { onStatusChange, count, value } = props;
    const [menuList, setMenuList] = useState(List.menuDetails["SOUP STATION"]);
    ...
    useEffect(() => {
        if (value.length > 0) {
            ...
            setMenuList(List.menuDetails["SOUP STATION"])
        }
    }, [value])
